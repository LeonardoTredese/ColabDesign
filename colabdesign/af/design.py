...
from colabdesign.shared.utils import copy_dict, update_dict, Key, dict_to_str, softmax, categorical, to_list, copy_missing
  ...
  def _save_results(self, aux=None, save_best=False,
    ...
    if save_best:
      ...
      if "metric" not in self._tmp["best"] or metric < self._tmp["best"]["metric"]:
        self._tmp["best"]["aux"] = copy_dict(aux, deep = False)
